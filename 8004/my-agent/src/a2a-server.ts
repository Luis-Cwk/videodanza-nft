...
import { streamResponse, generateResponse, generateContract, type AgentMessage } from './agent.js';
...
async function handleMessageSend(
  ...
  if (isContractRequest) {
    if (userText.toLowerCase().includes('nft') || userText.toLowerCase().includes('erc721')) {
      contractType = 'nft';
    } else if (userText.toLowerCase().includes('token') || userText.toLowerCase().includes('erc20')) {
      contractType = 'token';
    } else if (userText.toLowerCase().includes('marketplace') || userText.toLowerCase().includes('mercado')) {
      contractType = 'marketplace';
    } else if (userText.toLowerCase().includes('dao')) {
      contractType = 'dao';
    }
  }
  ...
  if (streaming && res) {
    ...
    if (isContractRequest && contractType) {
      // For contracts, generate all at once (no streaming)
      const contractCode = generateContract(contractType, contractParams);
      ...
      // Regular streaming for normal conversations
  ...
  if (isContractRequest && contractType) {
    // Generate smart contract using template + AI explanation
    const contractCode = generateContract(contractType, contractParams);
    const aiResponse = await generateResponse(
      `El usuario quiere crear un contrato tipo ${contractType}. Explícale qué hace este contrato y dale el código:\n\n${contractCode}\n\nAdemás, explica: 1) Cómo desplegarlo en Remix IDE, 2) Parámetros importantes a configurar, 3) Costos de gas aproximados en Sepolia`,
      history
